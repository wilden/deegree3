...
public class OGCFrontController extends HttpServlet {
    ...
    private static final Version SUPPORTED_CONFIG_VERSION = Version.parseVersion( "0.4.0" );
    ...
    protected void doPost( HttpServletRequest request, HttpServletResponse response )
        ...
        try {
            ...
            List<FileItem> multiParts = checkAndRetrieveMultiparts( request );
            if ( isKVP ) {
                String queryString = readPostBodyAsString( request );
                ...
                String dummySystemId = "HTTP Post request from " + request.getRemoteAddr() + ":"
                                       + request.getRemotePort();
                XMLStreamReader xmlStream = XMLInputFactory.newInstance().createXMLStreamReader( dummySystemId, pbr );
