...
public class GetRecordsHandler {
    ...
    private void searchResult( XMLStreamWriter writer, GetRecords getRec, Version version )
        ...
        if ( VERSION_202.equals( version ) ) {
            ...
            TransformatorPostGIS filterExpression = new TransformatorPostGIS( getRec.getConstraint() );

            // PostGISWhereBuilder filterExpression = new PostGISWhereBuilder( ISO_DC_Mappings, filter, sortCrit,
            // useLegacyPredicates );

            RecordStoreOptions gdds = new RecordStoreOptions( filterExpression.getWriter(), getRec.getResultType(),
