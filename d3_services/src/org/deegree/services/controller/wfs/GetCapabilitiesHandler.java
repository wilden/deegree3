    ...
    public void export110()
        ...
        if ( sections == null || sections.contains( "FEATURETYPELIST" ) ) {
            ...
            for ( FeatureType ft : servedFts ) {
                ...
                // wfs:OtherSRS
                for ( CRS otherSrs : querySRS ) {
                    if ( fs.getStorageSRS() == null || !otherSrs.getName().equals( fs.getStorageSRS().getName() ) ) {
                        writeElement( writer, WFS_NS, "OtherSRS", otherSrs.getName() );
                    }
                }
