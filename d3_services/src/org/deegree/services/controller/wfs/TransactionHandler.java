...
public class TransactionHandler {
    ...
    private void doInsert( Insert insert, Lock lock )
        ...
        // TODO: propagate srs to feature store?
        if ( insert.getSRSName() != null ) {
            CRS crs = new CRS( insert.getSRSName() );
            try {
                crs.getWrappedCRS();
            } catch ( UnknownCRSException e ) {
                String msg = "Cannot perform insert. Specified srsName '" + crs.getName()
        ...
        try {
            FeatureCollection fc = parseFeaturesOrCollection( insert.getFeatures() );
    ...
    private FeatureCollection parseFeaturesOrCollection( XMLStreamReader xmlStream )
        ...
        GMLStreamReader gmlStream = GMLInputFactory.createGMLStreamReader( GMLVersion.GML_31, xmlStream );
        ...
        while ( replacementIter.hasNext() ) {
            ...
            if ( pt == null ) {
                throw new OWSException( "Cannot update property '" + propName + "' of feature type '" + ft.getName()
                                        + "'. The feature type does not define such a property.",
                                        OWSException.OPERATION_NOT_SUPPORTED );
            }
            XMLStreamReader xmlStream = replacement.getReplacementValue();
            if ( xmlStream != null ) {
                try {
                    GMLDocumentIdContext gmlIdContext = new GMLDocumentIdContext( GMLVersion.GML_31 );
                    xmlStream.require( START_ELEMENT, WFS_NS, "Value" );
                    GMLFeatureReader decoder = new GMLFeatureReader( GML_31, ft.getSchema(), gmlIdContext );
                    newProperties.add( decoder.parseProperty( new XMLStreamReaderWrapper( xmlStream, null ), pt, null,
                                                              1 ) );
                ...
                newProperties.add( new GenericProperty<Object>( pt, null ) );
            }
        }
    ...
    private void sendResponse( Transaction request, HttpResponseBuffer response )
