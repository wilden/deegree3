...
import static org.deegree.crs.CRSRegistry.lookup;
    ...
    private void writeLayers( XMLStreamWriter writer, Layer layer )
        ...
        try {
            ...
            if ( layer.getBbox() != null ) {
                Envelope bbox = (Envelope) new GeometryTransformer( latlon ).transform( layer.getBbox() );
            ...
            for ( CRS crs : layer.getSrs() ) {
                ...
                Envelope envelope;
                try {
                    Envelope src = layer.getBbox();
                    if ( src.getCoordinateSystem() == null ) {
                        envelope = (Envelope) new GeometryTransformer( crs.getWrappedCRS() ).transform(
                                                                                                        layer.getBbox(),
                                                                                                        latlon );
                    } else {
                        envelope = (Envelope) new GeometryTransformer( crs.getWrappedCRS() ).transform( layer.getBbox() );
                    }
                } catch ( IllegalArgumentException e ) {
                    LOG.info( "Cannot transform:", e );
                    continue;
            ...
            LOG.warn( "Cannot find:", e );
        } catch ( IllegalArgumentException e ) {
            LOG.warn( "Cannot transform:", e );
        } catch ( TransformationException e ) {
            LOG.warn( "Cannot transform:", e );
        }
