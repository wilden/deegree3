...
public class GetFeatureInfo {
    ...
    private HashMap<Integer, Envelope> clickBoxes = new HashMap<Integer, Envelope>();
     ...
     */
    public Envelope getClickBox( int radius ) {
        if ( clickBoxes.get( radius ) != null ) {
            return clickBoxes.get( radius );
        }
        double w = bbox.getSpan0() / width;
        double h = bbox.getSpan1() / height;
        int r2 = radius / 2;
        r2 = r2 == 0 ? 1 : r2;
        Envelope res = fac.createEnvelope( new double[] { bbox.getMin().get0() + ( x - r2 ) * w,
                                                         bbox.getMax().get1() - ( y + r2 ) * h },
                                           new double[] { bbox.getMin().get0() + ( x + r2 ) * w,
                                                         bbox.getMax().get1() - ( y - r2 ) * h }, crs );
        clickBoxes.put( radius, res );
        return res;
    }
