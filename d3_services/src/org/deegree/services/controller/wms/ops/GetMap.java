...
import static org.deegree.commons.utils.MapUtils.calcScaleWMS130;
...
public class GetMap {
    ...
    public GetMap( Map<String, String> map, Version version, MapService service ) throws OWSException {
        ...
        try {
            scale = calcScaleWMS130( width, height, bbox, crs.getWrappedCRS() );
    ...
    static HashMap<String, List<?>> parseDimensionValues( Map<String, String> map )
        ...
        try {
            for ( String key : map.keySet() ) {
                if ( key.equals( "TIME" ) ) {
                    dims.put( "time", (List<?>) parseTyped( parseDimensionValues( map.get( key ), "time" ), true ) );
                }
                if ( key.equals( "ELEVATION" ) || key.startsWith( "DIM_" ) ) {
                    String name = key.equals( "ELEVATION" ) ? "elevation" : key.substring( 4 ).toLowerCase();
                    dims.put( name, (List<?>) parseTyped( parseDimensionValues( map.get( key ), name ), false ) );
                }
            }
            return dims;
        } catch ( ParseException e ) {
