...
import static org.deegree.crs.CRSCodeType.getUndefined;
...
public class FeatureLayer extends Layer {
    ...
    public FeatureLayer( String name, String title, Layer parent, String file ) throws FileNotFoundException,
                            IOException {
        super( name, title, parent );
        datastore = new ShapeFeatureStore( file, null, null );
        try {
            datastore.init();
        } catch ( FeatureStoreException e ) {
    ...
    public LinkedList<String> collectQueries( Style style, final GetMap gm, LinkedList<Query> queries )
        ...
        if ( featureType == null ) {
            queries.addAll( map( datastore.getSchema().getFeatureTypes( false, false ),
    ...
    public Pair<FeatureCollection, LinkedList<String>> getFeatures( final GetFeatureInfo fi, Style style )
        ...
        try {
            ...
            final Envelope clickBox = fi.getClickBox( 1 );
            ...
            if ( featureType == null ) {
                List<Query> queries = map( datastore.getSchema().getFeatureTypes( false, false ),
                                           new Mapper<Query, FeatureType>() {
                                               public Query apply( FeatureType u ) {
                                                   return new Query( u.getName(), clickBox, buildFilter( operator, u,
                                                                                                         clickBox ),
                                                                     fi.returnGeometries(), -1 );
                                               }
                                           } );
                ...
                Query query = new Query( featureType, clickBox,
                                         buildFilter( operator, datastore.getSchema().getFeatureType( featureType ),
                                                      clickBox ), fi.returnGeometries(), -1 );
    ...
    protected Pair<Filter, LinkedList<String>> getDimensionFilter( HashMap<String, List<?>> dims )
        ...
        if ( time != null ) {
            ...
            for ( Object o : vals ) {
                ...
                if ( o instanceof DimensionInterval<?, ?, ?> ) {
                    ...
                    os[i++] = new PropertyIsBetween( property, new Literal<String>( min ), new Literal<String>( max ),
                                                     false );
                ...
                if ( theVal != null ) {
                    ...
                    Literal<String> lit = new Literal<String>( formatISO8601DateWOMS( theVal ) );
        ...
        for ( String name : dimensions.keySet() ) {
            ...
            for ( Object o : vals ) {
                ...
                if ( o instanceof DimensionInterval<?, ?, ?> ) {
                    DimensionInterval<?, ?, ?> iv = (DimensionInterval<?, ?, ?>) o;
                    final String min = Double.toString( (Double) iv.min );
                    final String max = Double.toString( (Double) iv.max );
                    os[i++] = new PropertyIsBetween( property, new Literal<String>( min ), new Literal<String>( max ),
                                                     false );
                } else {
